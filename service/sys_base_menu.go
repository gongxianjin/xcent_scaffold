...
func DeleteBaseMenu(id float64) (err error) {
	err = lib.GORMDefaultPool.Preload("Parameters").Where("parent_id = ?", id).First(&model.SysBaseMenu{}).Error
	if err != nil {
		var menu model.SysBaseMenu
		db := lib.GORMDefaultPool.Preload("SysAuthoritys").Where("id = ?", id).First(&menu).Delete(&menu)
		err = lib.GORMDefaultPool.Delete(&model.SysBaseMenuParameter{}, "sys_base_menu_id = ?", id).Error
		if len(menu.SysAuthoritys) > 0 {
			err = lib.GORMDefaultPool.Model(&menu).Association("SysAuthoritys").Delete(&menu.SysAuthoritys).Error
		} else {
			err = db.Error
		}
	} else {
...
func GetBaseMenuById(id float64) (err error, menu model.SysBaseMenu) {
	err = lib.GORMDefaultPool.Preload("Parameters").Where("id = ?", id).First(&menu).Error
	return
}
