...
var Apis = []model.SysApi{
	...
	{gorm.Model{ID: 60, CreatedAt: time.Now(), UpdatedAt: time.Now()}, "/autoCode/getColumn", "获取所选table的所有字段", "autoCode", "GET"},
...
func InitSysApi(db *gorm.DB) {
	...
	for _, api := range Apis {
		if err := db.Save(&api).Error; err != nil { // 遇到错误时回滚事务
			db.Rollback()
			log.Fatal(err)
		}
	}
	db.Commit()
	log.Println("init sysApi success")
}
